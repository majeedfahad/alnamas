<div class="col-md-4 mb-2">
    ...
    object-fit: cover;"
        ...
        <div class="actions p-2">
            <ul class="d-flex">
                <li class="p-1 m-1"><a
                        href="{{route('best-images.toggleLike', ['image' => $image])}}#img-{{$image->id}}">
                ...
                @can('delete', $image)
                    <form action="{{route('best-images.destroy', ['best_image' => $image])}}"
                        ...
                        @csrf
                        @method('DELETE')
                        <li class="p-1 m-1 show_confirm">
                            <button type="submit" class="fa fa-trash" data-toggle="tooltip" style="font-size:15px;color:red; border: none;
...
<script>
    function showConfirm(event) {
        ...
        swal({
            title: `أكيد بتحذفها؟ ترا مافيه تراجع`,
            icon: "warning",
            dangerMode: true,
            buttons: ["خلاص هونت", "امسحها"],
        })
            .then((willDelete) => {
                if (willDelete) {
                    event.target.submit();
