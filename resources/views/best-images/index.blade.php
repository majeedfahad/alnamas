...
@section('content')
    <div class="best-image-form  my-3">
        <form method="post" action="{{route('best-images.store')}}" enctype="multipart/form-data" class="d-flex align-items-center pt-2 justify-content-between">
            @csrf
            <img src="{{auth()->user()->getFirstMediaUrl()}}" class="rounded-circle ml-1" style="width: 50px; height: 40px">

            <label for="fileInput" class="file-input-label btn btn-info mx-1 text-whiteق" style="width: 100%">
